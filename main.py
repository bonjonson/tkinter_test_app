...
# документация tkinter по ссылке https://ru.wikibooks.org/wiki/GUI_Help/Tkinter_book
import sqlite3
...
class Main(tk.Frame):
    ...
    def init_main(self):
        toolbar = tk.Frame(bg='#d7d8e0', bd=2)  # создаем тулбар
        toolbar.pack(side=tk.TOP, fill=tk.X)

        '''
        создаем кнопку, в аргументах перечисляем
        окно, к которому применяем; текст; вызов команды; фон; отступы; расположение картинки на кнопке; изображение
        '''

        # кнопка добавления новых позиций
        self.add_img = tk.PhotoImage(file='add.png')  # добавляем иконку
        ...
        btn_open_dialog.pack(side=tk.LEFT)  # создаем кнопку и указываем выравнивание

        # кнопка редактирования выделенной позиции
        ...
        btn_delete = tk.Button(toolbar, text='Удалить позицию', command=self.delete_records,
                                    bg='#d7d8e0', bd=0, compound=tk.TOP, image=self.delete_image)
        btn_delete.pack(side=tk.LEFT)  # создаем кнопку и указываем выравнивание

        # кнопка поиска
        self.search_image = tk.PhotoImage(file='search.png')
        btn_search = tk.Button(toolbar, text='Поиск', command=self.open_search_dialog,
                               bg='#d7d8e0', bd=0, compound=tk.TOP, image=self.search_image)
        btn_search.pack(side=tk.LEFT)  # создаем кнопку и указываем выравнивание
        ...
        btn_refresh = tk.Button(toolbar, text='Обновить', command=self.view_records,
                               bg='#d7d8e0', bd=0, compound=tk.TOP, image=self.refresh_image)
        btn_refresh.pack(side=tk.LEFT)  # создаем кнопку и указываем выравнивание
    ...
    def open_dialog(self):  # вызывает дочернее окно
        Child()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        self.geometry('400x220+400+300')  # устанавливаем размер окна
        ...
        self.grab_set()  # перехватывает события в приложении
